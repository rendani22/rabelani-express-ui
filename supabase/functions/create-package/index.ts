  ...
  try {
    ...
    if (poAllocations.length > 0) {
      ...
      if (missingAllocation) {
        const rollbackError = await rollbackPackage(adminClient, newPackage.id)
        const rollbackDetails = rollbackError
          ? ` Package rollback failed: ${rollbackError}`
          : ''
        return new Response(
          JSON.stringify({
            error: `Purchase order allocation references a package item that was not inserted (item_index: ${missingAllocation.item_index}).${rollbackDetails}`
          }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
      ...
      const { error: allocationError } = await adminClient.rpc(
      ...
      if (allocationError) {
        ...
        const rollbackDetails = rollbackError
          ? ` Package rollback failed: ${rollbackError}`
          : undefined
        const status = allocationError.code === 'P0001' ? 400 : 500
        return new Response(
          JSON.stringify({
            error: allocationError.code === 'P0001'
              ? allocationError.message
              : 'Failed to persist purchase order allocations',
            details: allocationError.code === 'P0001'
              ? rollbackDetails
              : [allocationError.message, rollbackDetails].filter(Boolean).join(' | ')
          }),
