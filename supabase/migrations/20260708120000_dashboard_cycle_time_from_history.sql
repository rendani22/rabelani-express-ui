-- ============================================================================
-- Fix Cycle-Time Insights: derive stage timestamps from status history
--
-- Problem: the dashboard's "Cycle-Time Insights" card only ever showed the
-- "Full cycle" figure; the three stage rows (Created → Picked up, Pickup →
-- Received, Received → Collected) were always blank. The lifecycle CTE computed
-- those stages from packages.picked_up_at / packages.received_at, but nothing
-- ever writes those two columns (update-package sets picked_up_by and
-- collected_at only), so they are always NULL. Full cycle worked because it
-- uses collected_at, which IS written.
--
-- Fix: derive each package's pickup/received/collected instants from
-- package_status_history.changed_at — the trigger records a row on every status
-- change, so this works for historical AND future orders without touching the
-- edge functions. Pickup = first time status became 'in_transit'; received =
-- first 'ready_for_collection'; collected prefers the stored collected_at and
-- falls back to the first 'collected' history row.
--
-- CREATE OR REPLACE replaces the whole function, so the full body from
-- 20260701200000_dashboard_metrics_returns.sql is reproduced here with a new
-- `pkg_transitions` CTE and a reworked `lifecycle` CTE.
-- ============================================================================
