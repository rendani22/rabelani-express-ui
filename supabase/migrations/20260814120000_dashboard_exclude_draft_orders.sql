-- ============================================================================
-- Draft orders: counted on their own, out of every operational metric
--
-- A draft is an order that has been captured but deliberately not released --
-- "save as draft (don't notify the receiver yet)". It has no receiver contact,
-- no SLA clock, and nothing to fulfil. Until now the ops dashboard's working
-- set (`pkg`) included drafts, so they silently inflated `total`, the daily
-- series, the heatmap, top receivers, location distribution and the returns
-- denominator -- while having no bucket of their own in `statusCounts`. The
-- Total Packages tile therefore never reconciled with the status tiles beside
-- it, and every rate computed against `total` (collection rate on both
-- dashboards, return rate on the executive one) was deflated by orders that
-- were never in the pipeline.
--
-- This migration makes the split explicit:
--   * `pkg` -- the working set behind every package-derived metric -- excludes
--     drafts, so `total`, the series, the heatmap, receivers, locations,
--     returns and top-shipped-items are all "live orders" and agree with one
--     another. `total` is now the exact sum of the status buckets.
--   * a new `drafts` CTE counts them under the same date + company scope, and
--     `stats.drafts` reports that count so the UI can show it and say what the
--     headline excludes.
--   * driverStats.totalPackages ("Total handled") likewise skips drafts -- a
--     draft has never been handled by anyone.
--
-- Metrics that were already draft-free are untouched: the SLA/stuck query
-- filters to the four open statuses, POD compliance and lifecycle require a
-- completed order, and driver performance requires a pickup.
--
-- `statusCounts` deliberately gains no `draft` key: it feeds the pipeline donut
-- and drafts are not a pipeline stage. Their count travels in `stats.drafts`.
--
-- CREATE OR REPLACE takes the whole body, so the function is reproduced in full
-- below; it is unchanged from 20260721170000 apart from the above.
-- ============================================================================
...
BEGIN
  ...
  WITH
  -- Date-filtered, non-deleted, released packages: the working set for most
  -- metrics. Drafts are excluded here rather than status-by-status downstream,
  -- so no metric can accidentally count an order that was never released.
  pkg AS (
    SELECT *
  ...
  -- ── Drafts: same scope as `pkg`, counted but never mixed into it ────────
  drafts AS (
  ...
  driver_counts AS (
    SELECT
      ...
      -- "Total handled": a draft has never been handled by a driver.
      (SELECT count(*) FROM public.packages
         WHERE deleted_at IS NULL AND status <> 'draft')                 AS total_packages
  ),
  ...
  SELECT jsonb_build_object(
    'stats', (SELECT jsonb_build_object(
        ...
        -- Captured but not released. Outside every other figure in this object.
        'drafts', (SELECT draft_count FROM drafts)
      ) FROM stats),
