...
SELECT
  ...
  COALESCE(SUM(poa.allocated_quantity), 0)::integer AS allocated_quantity,
  (poi.ordered_quantity - COALESCE(SUM(poa.allocated_quantity), 0))::integer AS remaining_quantity
FROM public.purchase_order_items poi
