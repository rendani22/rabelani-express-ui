-- ============================================================================
-- Signals: a POD counts when the document exists
--
-- WHY THIS IS A SEPARATE MIGRATION
-- Same reason as 20260721150000: a version already applied cannot be corrected
-- by editing it. This reproduces the whole function, so whichever of
-- 20260721140000 / 20260721150000 actually landed, applying this converges the
-- database to the intended state.
--
-- THE DEFECT
-- POD compliance -- and therefore Perfect Order Rate -- read exactly 0.0%.
--
-- The test was `has_pod AND locked AND (NOT is_delivery OR has_photo)`, where
-- `locked` is pods.is_locked. That column defaults to false and NOTHING in the
-- application ever sets it to true: mark-collected-dialog inserts is_locked
-- false, and no other write path touches it. The lock machinery around it is
-- real -- a trigger stamps locked_at, RLS policies on packages key off locked
-- PODs, update-package refuses to edit one -- but nothing engages it. So the
-- clause was `AND false` for every row.
--
-- That is not a metric reading zero because the depot is failing. It is a
-- metric gated on a flag the product does not write, which is worse: the whole
-- dashboard reported 0.0% and blamed "no compliant POD" for all of it, because
-- `clean` requires NOT pod_gap and pod_gap was unconditionally true.
--
-- THE FIX
-- A POD counts when the document exists. That is the bar the depot actually
-- works to, and the POD is what settles a dispute whether or not a lock flag
-- was ever flipped:
--
--     pod_gap := NOT has_pod
--
-- The photo is NOT dropped -- it is measured, it just no longer gates the
-- headline. `photoRate` (deliveries carrying a picture, over deliveries) is
-- still reported and still shown on the tile, so a delivery evidenced by
-- signature alone is still visible; it is simply not counted as "no POD",
-- which it plainly is not. `locked` is still reported too, so if POD locking
-- is ever wired up the figure starts working on its own.
--
-- Everything else is unchanged, including the pod_scope denominator from
-- 20260721150000 (returns excluded -- a return has no delivery to evidence).
-- ============================================================================
...
BEGIN
  ...
  verdict AS (
    SELECT
      ...
      -- pods.is_locked: nothing in the product ever sets it, so including it
      -- made this false for every order (see the header). The photo is
