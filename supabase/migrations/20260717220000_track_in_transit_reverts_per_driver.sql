-- ============================================================================
-- Track the nightly in-transit → notified revert, per driver
--
-- Context: `revert_stale_in_transit()` (20260708180000) flips every order still
-- `in_transit` at 20:00 SAST back to `notified` so it re-enters the dispatch
-- queue. It is the operational signal for "a driver took this and did not
-- complete it today" — but it was unmeasurable, because the same UPDATE that
-- flips the status also does:
--
--     picked_up_by = NULL, status_changed_by = NULL
--
-- i.e. it destroys the driver attribution at the exact instant that
-- attribution becomes interesting. package_status_history records the
-- `notified` row with changed_by = NULL ("System"), so not even the timeline
-- can say who was holding it. Every past revert is already unrecoverable; this
-- migration stops the bleeding from tonight forward.
--
--   1. `package_revert_events` — an append-only fact table. One row per order
--      per revert, capturing the driver BEFORE the column is cleared.
--   2. `revert_stale_in_transit()` — rewritten to snapshot-then-update in a
--      single statement, so the write and the record cannot drift apart.
--   3. `get_dashboard_metrics` — `driverPerformance` rows gain
--      `revertedLast30Days` and `revertedAllTime`.
--
-- The revert RULE is deliberately unchanged: it still reverts every in-transit
-- order with no minimum age, so an order picked up at 19:55 is reverted at
-- 20:00. That is intended ("end of day means end of day"). `hoursInTransit` is
-- captured per event so that late-pickup noise stays visible in the data
-- rather than being silently folded into a driver's count.
-- ============================================================================

-- ── 1. The fact table ───────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.package_revert_events (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  package_id       uuid NOT NULL REFERENCES public.packages(id)  ON DELETE CASCADE,
  -- The driver who held the order, read from packages.picked_up_by before it
  -- was cleared. Nullable: an order can be in_transit with no driver attached,
  -- and ON DELETE SET NULL means removing a user keeps the event itself.
  driver_user_id   uuid REFERENCES auth.users(id) ON DELETE SET NULL,
  reverted_at      timestamptz NOT NULL DEFAULT now(),
  -- When this leg of transit began (latest in_transit row in status history),
  -- and the resulting dwell. NULL when history has no in_transit row.
  picked_up_at     timestamptz,
  hours_in_transit numeric(10,2)
);

COMMENT ON TABLE public.package_revert_events IS
  'Append-only log of the nightly in-transit → notified revert. One row per '
  'order per revert. Exists because revert_stale_in_transit() clears '
  'packages.picked_up_by, so this is the only durable record of which driver '
  'was holding a reverted order.';

CREATE INDEX IF NOT EXISTS package_revert_events_driver_reverted_at_idx
  ON public.package_revert_events (driver_user_id, reverted_at DESC);
CREATE INDEX IF NOT EXISTS package_revert_events_package_id_idx
  ON public.package_revert_events (package_id);
CREATE INDEX IF NOT EXISTS package_revert_events_reverted_at_idx
  ON public.package_revert_events (reverted_at DESC);

ALTER TABLE public.package_revert_events ENABLE ROW LEVEL SECURITY;

-- Read-only to staff, matching package_status_history. No INSERT/UPDATE/DELETE
-- policy: the only writer is revert_stale_in_transit(), which is SECURITY
-- DEFINER and bypasses RLS. Nothing should ever amend this log by hand.
DROP POLICY IF EXISTS "Active staff can view revert events" ON public.package_revert_events;
CREATE POLICY "Active staff can view revert events" ON public.package_revert_events
  FOR SELECT TO authenticated USING (public.is_active_staff());

GRANT SELECT ON public.package_revert_events TO authenticated;


-- ── 2. Revert, now recording what it destroys ───────────────────────────────
-- Snapshot and update in ONE statement: every CTE sees the same pre-UPDATE
-- snapshot, so `snapshot` reads picked_up_by while `upd` is clearing it. The
-- alternative (SELECT, then UPDATE) could disagree if a driver picked up an
-- order between the two. FOR UPDATE locks the rows for the whole statement.
--
-- Note RETURNING cannot help here: it yields the NEW row, whose picked_up_by
-- is already NULL — the very value being preserved.
...
BEGIN
  ...
  WITH snapshot AS (
    SELECT id, picked_up_by
    FROM public.packages
    WHERE status = 'in_transit'
      AND deleted_at IS NULL
    FOR UPDATE
  ),
  -- Start of the CURRENT transit leg: an order can be picked up, reverted, and
  -- picked up again, so max() (not min()) is the leg being closed right now.
  legs AS (
    SELECT s.id, max(h.changed_at) AS picked_up_at
    FROM snapshot s
    LEFT JOIN public.package_status_history h
      ON h.package_id = s.id AND h.status = 'in_transit'
    GROUP BY s.id
  ),
  upd AS (
  ...
  ins AS (
    INSERT INTO public.package_revert_events
      (package_id, driver_user_id, reverted_at, picked_up_at, hours_in_transit)
    SELECT
      s.id,
      s.picked_up_by,
      now(),
      l.picked_up_at,
      CASE WHEN l.picked_up_at IS NULL THEN NULL
           ELSE round((EXTRACT(epoch FROM (now() - l.picked_up_at)) / 3600.0)::numeric, 2)
      END
    FROM snapshot s
    LEFT JOIN legs l ON l.id = s.id
    RETURNING 1
  )
...
-- ── 3. Dashboard: per-driver revert counts ──────────────────────────────────
-- CREATE OR REPLACE replaces the whole function, so the full body of
-- get_dashboard_metrics is reproduced here (unchanged from
-- 20260717160000_pod_delivery_photo_compliance.sql apart from the
-- revert_counts / perf_uids / perf changes).
