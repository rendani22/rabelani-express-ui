-- ============================================================================
-- Signals dashboard: Perfect Order Rate and the levers under it
--
-- The Operations dashboard answers "what is happening"; the Executive dashboard
-- answers "how is the business doing". Neither answers "which number needs me
-- today", because almost every figure on them is a whole number with no target
-- and no comparison -- `Total Packages: 12,431` cannot change anyone's
-- behaviour. This RPC backs a third view built only from metrics that can.
--
-- THE NORTH STAR
-- Perfect Order Rate: of the orders that reached a terminal state in the
-- window, the share that went through clean --
--
--     on time  AND  not returned  AND  with a compliant POD
--
-- plus a fourth, period-level leg: purchase orders Coupa sent that never became
-- a Global PO. Those are orders the depot was asked to fulfil and never even
-- recorded, so they cannot be perfect; counting them only in the denominator is
-- what stops the rate from improving when ingestion silently drops work.
--
--     Perfect Order Rate =        clean orders
--                          ────────────────────────────────
--                          terminal orders + dropped Coupa POs
--
-- WHY A ROLLING 28 DAYS AND NOT MONTH-TO-DATE
-- MTD against a full prior month is the most common dashboard lie: on the 3rd
-- of the month it compares three days against thirty and reports a collapse.
-- Every headline here is the last 28 days against the 28 before it, so the two
-- sides of every comparison are always the same length.
--
-- WHY PERCENTILES AND NOT AVERAGES
-- `avgTotalCycleHours` on the Operations tab reads fine at ~30h while a tail of
-- nine-day parcels generates every complaint the depot receives. A mean cannot
-- see that tail; p90 is the number the customer experiences, and p90 ÷ p50 says
-- whether the operation is consistent or merely lucky on average.
--
-- ATTRIBUTION
-- An order can fail several ways at once, so the "lost to" breakdown assigns
-- each failed order to exactly one cause in a fixed precedence -- returned,
-- then late, then POD gap. The buckets therefore partition the failed set and
-- the points sum to (100 − rate) rather than double-counting past it. Returned
-- outranks late because a returned order's lateness is moot: the fix is the
-- return cause.
--
-- COMPANY SCOPE
-- Every per-order metric scopes by receiver_id → receiver_profiles.company_id,
-- as the other dashboards do. The Coupa leg does not and cannot: a *failed*
-- ingestion has no purchase order and therefore no customer, which is often the
-- very reason it failed. Scoping it would report 0% dropped however much was
-- being dropped, so when a company is selected the leg is marked unavailable
-- and drops out of the denominator entirely -- the same treatment the Executive
-- tab's ingestion card already gets.
-- ============================================================================

-- ── Permission ──────────────────────────────────────────────────────────────
-- Sensitive: the view carries revenue-at-risk and per-driver figures, and the
-- RPC checks the key itself rather than trusting the tab to be hidden (matching
-- get_dashboard_metrics and get_coupa_ingestion_report). Active admins hold
-- everything unconditionally via has_permission(), so only manager is seeded.
INSERT INTO public.permissions (key, feature, label, description, is_sensitive, sort_order)
VALUES (
  'dashboard.signals.view',
  'Dashboards',
  'View signals dashboard',
  'Perfect Order Rate, SLA and POD compliance, revenue at risk, per-driver reverts.',
  true,
  102
)
...
VALUES ('manager', 'dashboard.signals.view')
...
BEGIN
  ...
  pod_pkg AS (
    SELECT
      ...
      COALESCE(d.is_locked, false)                          AS locked,
  ...
  verdict AS (
    SELECT
      ...
      NOT (pp.has_pod AND pp.locked AND (NOT pp.is_delivery OR pp.has_photo)) AS pod_gap,
      pp.is_delivery,
      pp.has_photo,
      pp.locked
    FROM term_win t
  ...
  cur_agg AS (
    SELECT
      ...
      count(*) FILTER (WHERE NOT pod_gap)                                       AS pod_ok,
      count(*) FILTER (WHERE locked)                                            AS pod_locked,
  ...
  SELECT jsonb_build_object(
    ...
    'inputs', (
      SELECT jsonb_build_object(
        ...
        'podCompliance', jsonb_build_object(
          'rate',       CASE WHEN c.orders > 0
                             THEN round((c.pod_ok::numeric * 100) / c.orders, 1) ELSE NULL END,
          'priorRate',  CASE WHEN p.orders > 0
                             THEN round((p.pod_ok::numeric * 100) / p.orders, 1) ELSE NULL END,
    ...
    'series', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'pod',       CASE WHEN s.orders > 0
                          THEN round((s.pod_ok::numeric * 100) / s.orders, 1) ELSE NULL END,
...
COMMENT ON FUNCTION public.get_signal_metrics(INTEGER, UUID) IS
  'Signals dashboard: Perfect Order Rate (on time AND not returned AND compliant '
  'POD, with dropped Coupa POs in the denominator), its four input levers, live '
  'guardrails and a small business layer. Headlines compare a rolling 28 days '
  'against the preceding 28. Requires dashboard.signals.view.';

GRANT EXECUTE ON FUNCTION public.get_signal_metrics(INTEGER, UUID) TO authenticated;

-- The dwell calculation reads every history row for a terminal order, ordered
-- by time within the package. The existing package_id index leaves the sort to
-- a separate step on a table that only grows; this serves the window function's
-- PARTITION BY / ORDER BY directly.
CREATE INDEX IF NOT EXISTS package_status_history_package_changed_idx
  ON public.package_status_history USING btree (package_id, changed_at);
