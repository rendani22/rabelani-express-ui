-- ============================================================================
-- Signals: POD compliance measures completed orders, not every terminal order
--
-- WHY THIS IS A SEPARATE MIGRATION
-- 20260721140000_signal_metrics.sql has already been applied. Supabase records
-- applied migrations by version, so editing that file changes nothing on a
-- database that has already run it; it is left alone and the correction ships
-- here. CREATE OR REPLACE replaces the whole function, so the full body is
-- reproduced below -- the convention the dashboard migrations already follow.
--
-- THE DEFECT
-- The POD compliance lever divided by `c.orders` -- every order that reached a
-- terminal state, collected AND returned. A returned order was never delivered,
-- so it has no POD, so every return scored as a POD failure.
--
-- That let a bad returns month drag down a figure that is supposed to be about
-- drivers capturing signatures: 500 orders finish, 15 returned, all 485
-- collected ones flawless -> 97.0%, under the 98% target, amber, with nothing
-- actually wrong. It also disagreed with the Executive tab's POD card, which
-- measures against completed orders only -- two screens in one product showing
-- different "POD compliance" figures.
--
-- THE FIX
-- A new `pod_scope` = terminal orders excluding returns. `pod_ok`, `pod_locked`,
-- the headline rate, the prior-period rate, the weekly series and `total` all
-- divide by it. `photoRate` was already correct: its denominator is deliveries,
-- and a returned order is never completion_status = 'Delivered'.
--
-- On-time and first-time-right still divide by `c.orders`, which is right for
-- them -- a return is exactly what first-time-right exists to count.
--
-- The North Star is unchanged by this. `clean` and `lost_pod` both already
-- excluded returns, so a return was attributed to the returned bucket and never
-- counted against POD as well; the leak breakdown never double-counted.
--
-- This migration is idempotent and converges any deployed variant of the
-- function to the intended state, so it does not matter which revision of
-- 20260721140000 actually landed.
-- ============================================================================
...
BEGIN
  ...
  cur_agg AS (
    SELECT
      ...
      count(*) FILTER (WHERE NOT returned AND locked)                           AS pod_locked,
