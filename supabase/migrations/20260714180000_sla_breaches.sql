-- ============================================================================
-- SLA breaches: live measurement, true total, and a full downloadable list
--
-- Three changes, all rooted in the same defect: "stuck" was computed from the
-- `pkg` CTE, which is filtered by `created_at` BETWEEN p_date_from/p_date_to.
-- But an SLA breach is measured from `updated_at` against now() — it is a live
-- fact, not a historical one. Scoping it by creation date means the oldest
-- orders (the worst breaches) drop out of the card the moment a date window is
-- applied. The dashboard does not send dates today, so this is currently
-- dormant, but it is a trap for the first person to add a date picker.
--
--   1. `sla_threshold_hours(status)` — the per-status threshold, previously a
--      CASE expression triplicated inside the stuck CTE, now one function so the
--      card, the total, and the export can never disagree.
--
--   2. `get_dashboard_metrics` — stuck now reads from a new `open_pkg` CTE:
--      every non-deleted open package, company-scoped, NOT date-filtered. The
--      card still shows the worst 10, but a new `stuckTotal` reports the real
--      count. The executive dashboard's "revenue at risk" KPI reads the length
--      of the 10-item array, so it has been silently capped at 10 — `stuckTotal`
--      is what fixes it. Expect that number to jump on deploy.
--
--   3. `get_sla_breaches(p_company_id)` — new RPC returning EVERY breaching
--      order with contact and timing detail, for the CSV export. Fetched lazily
--      on download so the dashboard payload stays small.
--
-- Receiver name/phone come from `receiver_profiles` (packages.receiver_id), not
-- `pods` — a breaching package has no POD by definition. LEFT JOINed so packages
-- without a linked profile still appear, with only the email.
--
-- CREATE OR REPLACE replaces the whole function, so the full body of
-- get_dashboard_metrics is reproduced here (unchanged from 20260708150000 apart
-- from the stuck/open_pkg/stuckTotal changes noted above).
-- ============================================================================

-- ── Per-status SLA threshold, in hours. NULL for statuses that cannot breach. ──
CREATE OR REPLACE FUNCTION public.sla_threshold_hours(p_status public.package_status)
RETURNS INT
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE p_status
    ...
    WHEN 'notified'             THEN 24
    WHEN 'in_transit'           THEN 24
    WHEN 'ready_for_collection' THEN 72
  END;
$$;

GRANT EXECUTE ON FUNCTION public.sla_threshold_hours(public.package_status) TO authenticated;
...
-- ============================================================================
-- get_sla_breaches — every breaching order, for the CSV export.
--
-- Same predicate as the dashboard's `breaching` CTE (via sla_threshold_hours),
-- but uncapped and with the contact/timing columns an operator needs to actually
-- work the list. No date parameters: a breach is always "as of now".
-- ============================================================================
