-- ============================================================================
-- Add returns analysis to get_dashboard_metrics
--
-- The Executive dashboard's Operations band needs returns as a real card
-- (rate + trend + by-location), not a bare count. The data already exists
-- (packages.status = 'returned', created_at, delivery_location_id); this
-- migration surfaces it by extending the dashboard aggregation RPC with a
-- `returns` block:
--   - returnedTotal / ordersTotal  → overall return rate
--   - monthly[]                    → last 6 months, returned + total per month
--   - byLocation[]                 → per delivery location, returned + total
--
-- "Returned in month X" is cohort-based: an order created in month X whose
-- CURRENT status is 'returned' counts against month X. Root cause is not
-- available — return reasons live only in free-text `packages.notes`.
--
-- CREATE OR REPLACE replaces the whole function, so the full body from
-- 20260629120000_dashboard_metrics_rpc.sql is reproduced here with the returns
-- CTEs + output key added.
-- ============================================================================
