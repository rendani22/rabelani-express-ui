...
describe('UserCardComponent', () => {
  ...
  describe('User Information Display', () => {
    it('should display user name', () => {
      const nameElement = debugElement.query(By.css('h2'));
      expect(nameElement.nativeElement.textContent.trim()).toBe(mockUser.name);
    });

    it('should display user avatar', () => {
      const avatarElement = debugElement.query(By.css('img'));
    ...
    it('should display user bio', () => {
      const bioElement = debugElement.query(By.css('p.line-clamp-2'));
      expect(bioElement.nativeElement.textContent.trim()).toBe(mockUser.bio);
    });

    it('should display country flag', () => {
      const flagElement = debugElement.query(By.css('span.text-sm.leading-none'));
      expect(flagElement.nativeElement.textContent.trim()).toBe(mockUser.countryFlag);
    });
  ...
  describe('Menu Functionality', () => {
    it('should toggle menu when menu button is clicked', () => {
      const menuButton = debugElement.query(By.css('button[aria-haspopup="true"]'));
    ...
    it('should display menu options when menu is open', () => {
      ...
      const menuItems = debugElement.queryAll(By.css('[role="menuitem"]'));
    ...
    it('should emit action event when menu option is clicked', () => {
      ...
      const mockOption = component.menuOptions()[0] as UserCardMenuOption;
  ...
  describe('Action Buttons', () => {
    it('should display send email button when showSendEmail is true', () => {
      ...
      const sendEmailButton = debugElement.query(By.css('a[aria-label^="Send email"]'));
    ...
    it('should display edit profile button when showEditProfile is true', () => {
      ...
      const editButton = debugElement.query(By.css('a[aria-label^="Edit profile"]'));
  ...
  describe('Accessibility', () => {
    ...
    it('should have proper alt text for avatar', () => {
      const avatar = debugElement.query(By.css('img'));
      expect(avatar.nativeElement.alt).toBe(mockUser.name);
    });

    it('should render menu with accessible label when opened', () => {
      component.isMenuOpen.set(true);
      fixture.detectChanges();
      const menu = debugElement.query(By.css('[role="menu"]'));
      expect(menu.nativeElement.getAttribute('aria-label')).toContain(mockUser.name);
    });
  });
});
