 ...
 *
 * Framework-agnostic port of the legacy Angular `core/models/package.models.ts`.
 * `PACKAGE_STATUS` / `PackageStatus` live in `@/lib/status` and are re-exported
 * here so existing import sites keep working.
 */

import { PACKAGE_STATUS, type PackageStatus } from '@/lib/status'

export { PACKAGE_STATUS, type PackageStatus } from '@/lib/status'
...
export type PackageActionResult =
  PackageServiceResult<PackageActionSuccessResponse>
...
export type GetPurchaseOrderByNumberResult =
  PackageServiceResult<PurchaseOrderLookupData>
...
export function getAllowedManualStatusTransitions(
  ...
  return PACKAGE_STATUS_FLOW.slice(currentIndex + 1).filter(
    (s) => s !== PACKAGE_STATUS.COLLECTED
  )
}
