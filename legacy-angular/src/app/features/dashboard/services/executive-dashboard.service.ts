...
const EMPTY_FUNNEL: FulfilmentFunnel = { stages: [], conversionRate: 0, totalInFunnel: 0 };
...
const EMPTY_BRIEFINGS: ExecutiveBriefingSet = {
  today: EMPTY_BRIEFING,
  week: EMPTY_BRIEFING,
  month: EMPTY_BRIEFING,
  year: EMPTY_BRIEFING,
};
...
export class ExecutiveDashboardService {
  ...
  private readonly _briefings = signal<ExecutiveBriefingSet>(EMPTY_BRIEFINGS);
  readonly briefings = this._briefings.asReadonly();

  private readonly _scorecard = signal<HealthMetric[]>([]);
  readonly scorecard = this._scorecard.asReadonly();

  private readonly _funnel = signal<FulfilmentFunnel>(EMPTY_FUNNEL);
  readonly funnel = this._funnel.asReadonly();

  private readonly _orderBook = signal<OrderBook>(EMPTY_ORDER_BOOK);
  readonly orderBook = this._orderBook.asReadonly();

  private readonly _inventory = signal<InventoryPosition>(EMPTY_INVENTORY);
  readonly inventory = this._inventory.asReadonly();

  private readonly _concentration = signal<CustomerConcentration>(EMPTY_CONCENTRATION);
  readonly concentration = this._concentration.asReadonly();

  // ── Operations layer ──────────────────────────────────────────────────────

  private readonly _geography = signal<DeliveryGeography>(EMPTY_GEOGRAPHY);
  readonly geography = this._geography.asReadonly();

  private readonly _podCompliance = signal<PodCompliance>(EMPTY_POD);
  readonly podCompliance = this._podCompliance.asReadonly();

  private readonly _returns = signal<ReturnsReport>(EMPTY_RETURNS);
  ...
  async load(force = false): Promise<void> {
    ...
    try {
      // Revenue is the primary payload; operational metrics and the PO order
      // book enrich the story but must never block or blank the revenue view,
      // so they degrade gracefully (settled, not awaited-all).
      const [revenueRes, opsRes, poRes] = await Promise.all([
        this.supabase.client.rpc('get_executive_metrics'),
        this.supabase.client.rpc('get_dashboard_metrics', { p_date_from: null, p_date_to: null }),
        this.supabase.client
          .from('purchase_orders')
          .select('status, po_value, po_date, created_at'),
      ]);
      ...
      this._funnel.set(this.buildFunnel(revenue, ops));
      this._inventory.set(this.buildInventory(ops));
      ...
      this._orderBook.set(this.buildOrderBook(poRows));

      // Operations band — delivery-side metrics from the ops payload. Built
      // before the narrative so the briefing can cite the returns trend.
      this._geography.set(this.buildGeography(ops));
      this._podCompliance.set(this.buildPodCompliance(ops));
      this._returns.set(this.buildReturns(ops));

      // Concentration + the derived scorecard and narrative need both payloads.
      this._concentration.set(this.buildConcentration(revenue));
      this._scorecard.set(this.buildScorecard(revenue, ops));
      this._briefings.set(this.buildBriefings(revenue, ops));
  ...
  /**
   * Builds the order → collection funnel. Counts come from the operational
   * status distribution; ZAR value is only known for the two stages the
   * revenue RPC prices (ready-for-collection and collected).
   */
  private buildFunnel(m: ExecutiveMetricsPayload, ops: DashboardOpsPayload | null): FulfilmentFunnel {
    const sc = ops?.statusCounts ?? {};
    const n = (k: string) => Math.max(0, Math.round(Number(sc[k] ?? 0)));

    const pending = n('pending') + n('notified');
    const transit = n('in_transit');
    const ready = n('ready_for_collection');
    const collected = n('collected');

    const raw: Array<Omit<FunnelStage, 'share'>> = [
      { key: 'pending', label: 'Pending', orders: pending, value: null, colorClass: 'bg-amber-500' },
      { key: 'transit', label: 'In Transit', orders: transit, value: null, colorClass: 'bg-blue-500' },
      { key: 'ready', label: 'Ready to Collect', orders: ready, value: m.summary?.pipelineValue ?? null, colorClass: 'bg-violet-500' },
      { key: 'collected', label: 'Collected', orders: collected, value: m.summary?.totalValue ?? null, colorClass: 'bg-emerald-500' },
    ];
  ...
  private buildInventory(ops: DashboardOpsPayload | null): InventoryPosition {
   ...
   */
  private buildGeography(ops: DashboardOpsPayload | null): DeliveryGeography {
  ...
  /**
   * Builds proof-of-delivery compliance. A missing POD means a delivery can't
   * be proven in a dispute, so a low capture rate is a cash-collection risk.
   */
  private buildPodCompliance(ops: DashboardOpsPayload | null): PodCompliance {
   ...
   * locations. Present only once the returns RPC extension is deployed; falls
   * back to unavailable so the card can explain itself. Root cause is not
   * captured (return reasons live in free-text notes only).
   */
  private buildReturns(ops: DashboardOpsPayload | null): ReturnsReport {
    const r = ops?.returns;
   ...
   */
  private buildOrderBook(rows: PurchaseOrderRow[]): OrderBook {
  ...
  private buildConcentration(m: ExecutiveMetricsPayload): CustomerConcentration {
  ...
  /**
   * Builds the 5-tile health scorecard spanning money and operations:
   * revenue (MTD Δ), awaiting-collection pipeline, fulfilment time, revenue at
   * risk (SLA breaches), and the collection (conversion) rate.
   */
  private buildScorecard(m: ExecutiveMetricsPayload, ops: DashboardOpsPayload | null): HealthMetric[] {
    const p = m.periods;
   ...
   * selectable window. Each period tells its own story — its revenue and
   * trend, the momentum behind it, and the single risk to watch — so the
   * reader can switch between day, week, month and year.
   */
  private buildBriefings(m: ExecutiveMetricsPayload, ops: DashboardOpsPayload | null): ExecutiveBriefingSet {
    const set = {} as ExecutiveBriefingSet;
    for (const spec of BRIEFING_PERIODS) {
      set[spec.key] = this.buildPeriodBriefing(spec, m, ops);
    }
    return set;
  }

  /**
   * Writes one period's briefing: a revenue headline, several supporting
   * stories (period-specific momentum first, then standing business context),
   * and the highest-priority watch item for that window.
   */
  private buildPeriodBriefing(
    spec: PeriodSpec,
    ...
    const conc = this.buildConcentration(m);
...
/** Compact ZAR for narrative strings (the pipe handles template display). */
